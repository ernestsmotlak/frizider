...
2. **Selection mode wired into all 4 surfaces**, each with: a "select all" button (shown only in select mode), a select-mode toggle button (checkbox icon ↔ X icon, violet gradient when active) next to the existing "+" add button, violet ring/border + checkmark-circle styling on selected items, and a sticky "Move N item(s)" button in violet (shown once ≥1 item is selected) that opens `ConvertItemsModal`:
